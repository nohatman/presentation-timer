...
io.on('connection', (socket) => {
  ...
  socket.on('startTimer', (data) => {
    ...
    if (data.endAtTarget) timerState.endAtTarget = data.endAtTarget;
  ...
  socket.on('resumeTimer', () => {
    if (timerState.mode === 'paused') {
      ...
      if (timerState.pauseTime && timerState.startTime) {
        const pauseDuration = timerState.pauseTime - timerState.startTime;
  ...
  socket.on('updateSettings', (data) => {
    ...
    // If setting end-at time, calculate duration
    if (data.endAtTarget) {
      const now = new Date();
